...
function getQueueItems(dbUI, collectionName, filter, callback) {
  ...
  try {
    ...
    if (numberPhs > 1) {
      ...
      return dbUI.find(collectionName, null, null, filter, (err, res) => {
        if (err) {
...
function claimTurn(dbUI, collectionName, queueItem, callback) {
  ...
  try {
    ...
    if (numberPhs > 1) {
      ...
      return dbUI.replaceOne(collectionName, { _id: data._id }, data, {}, (error, result) => {
        if (error) {
...
function freeQueueItem(dbUI, collectionName, queueItem, callback) {
  ...
  try {
    ...
    if (numberPhs > 1) {
      ...
      return dbUI.delete(collectionName, { _id: queueItem._id }, {}, false, (error, result) => {
        if (error) {
          ...
          */
          ...
          db.listCollections({ name: queueColl }).toArray((dberr, dbres) => {
            ...
            if (dberr || dbres === null || dbres === undefined || dbres.length <= 0) {
              ...
              this.dbUtil.createCollection(queueColl, (error, result) => {
                if (error) {
                ...
                this.dbUtil.createIndex(queueColl, { transNum: 1 }, {}, (inerr, inres) => {
                  if (inerr) {
              ...
              // Get the queue collection
              // MONGOQ = db.collection(queueColl);
              this.dbUtil.createCollection(queueColl, (error, res) => {
                if (error) {
              ...
              this.dbUtil.delete(queueColl, deleteFilter, {}, true, (delerr, res) => {
                if (delerr) {
  ...
  requestQueueItem(myRequest, transNum, priority, event, callback) {
    ...
    try {
      ...
      if (numberPhs > 1) {
        ...
        return this.dbUtil.countDocuments(queueColl, {}, {}, (gerror, qsize) => {
          if (gerror) {
          ...
          return this.dbUtil.create(queueColl, data, (error, result) => {
            if (error) {
  ...
  finishTurn(queueItem, reqEnd, callback) {
    ...
    try {
      ...
      if (numberPhs > 1) {
        ...
        // If the number of Pronghorns is greater than 1, the queue is in the database
        // return MONGOQ.replaceOne({ _id: data._id }, data, (error, result) => {
        return this.dbUtil.replaceOne(queueColl, { _id: data._id }, data, {}, (error, result) => {
          if (error) {
